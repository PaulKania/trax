...
def RawPolicy(seq_model, n_controls, n_actions):
  ...
  The resulting model takes as input observation and action sequences, but only
...
def SerializedPolicy(
  ...
  Returns:
    ...
    RawPolicy.
  """
  assert action_serializer.representation_length == n_controls, (
      'Action symbols should correspond 1-1 to controls.'
  )
  ...
  return tl.Serial([            # (obs, act)
      ...
      # Shift one symbol to the right for causality of action symbols.
      tl.ShiftRight(),
